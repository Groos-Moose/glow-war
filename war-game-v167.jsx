...
function WarPathRow({spoils,warCard,glow,isPlayer,fam,inWar,pathBg,pathBorder,revealed,warCollect,rw,pol,carpetDelta,warCardRef,onAnimDone,mercySpoils,mercySide,dw2Sp,dw2W,dw2Revealed,dw2Round}){
  ...
  return(
    <div style={{position:"absolute",inset:0,zIndex:200,background:"rgba(0,0,0,.92)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"linear-gradient(160deg,#1a0f05,#0f0a1a)",border:"2px solid rgba(218,165,32,.5)",borderRadius:18,padding:"22px 20px",width:"88%",maxWidth:340,maxHeight:"90vh",overflowY:"auto",position:"relative"}}>
        ...
        <div style={{fontSize:9,color:"rgba(218,165,32,.65)",textAlign:"center",letterSpacing:1.5,marginBottom:14}}>v167</div>
...
function StatsPanel({dur,fmt,fights,wars,pWarWins,cWarWins,longestSiege,mercyHits,royalsCaptured,playerTaps,warCardsWon,pTotal,cTotal,onClose}){
  const rows=[
    {cat:"VITALITY",label:"Remaining Lifespan",val:pTotal<=0?"--":`${pTotal} cards (${Math.round(pTotal/52*100)}%)`},
    {cat:"VITALITY",label:"Forfeit Risk",val:pTotal<=5?"CRITICAL":pTotal<=13?"HIGH":pTotal<=26?"MODERATE":"LOW"},
    {cat:"VITALITY",label:"Heartbeat Spikes",val:`${mercyHits} mercy event${mercyHits!==1?"s":""}`},
    {cat:"COMBAT",label:"Duels Fought",val:`${fights}`},
    {cat:"COMBAT",label:"Wars Won",val:`${pWarWins} of ${wars} wars`},
    {cat:"COMBAT",label:"War Win Rate",val:wars>0?`${Math.round(pWarWins/wars*100)}%`:"--"},
    {cat:"COMBAT",label:"Longest Siege",val:longestSiege>0?`${longestSiege} spoil${longestSiege!==1?"s":""}`:"-"},
    {cat:"COMBAT",label:"Royals Captured",val:`${royalsCaptured}`},
    {cat:"LUCK",label:"Luck Quotient",val:(()=>{const base=fights>0?Math.round((pWarWins/Math.max(wars,1))*50+(royalsCaptured/Math.max(fights,1))*100):0;return fights>0?`${Math.min(base,99)}/99`:"--";})()},
    {cat:"LUCK",label:"Suit Dominance",val:(()=>{const suits={"♠":"Spades","♥":"Hearts","♦":"Diamonds","♣":"Clubs"};const fav=pWarWins>cWarWins?"Offensive":"Defensive";return fav;})()},
    {cat:"PERFORMANCE",label:"CPM (Clicks/Min)",val:dur>0?`${Math.round(playerTaps/(dur/60))}`:"-"},
    {cat:"PERFORMANCE",label:"Card Point Value",val:`${warCardsWon} pts captured in war`},
    {cat:"PERFORMANCE",label:"War Efficiency",val:wars>0&&warCardsWon>0?`${Math.round(warCardsWon/wars)} cards/war avg`:"--"},
  ];
  let lastCat="";
  return(
              ...
              return(
                <React.Fragment key={i}>
                  ...
                  <tr style={{background:i%2===0?"rgba(255,255,255,.02)":"transparent"}}>
                    <td style={{padding:"4px 6px",color:"rgba(200,190,220,.7)",fontSize:10}}>{r.label}</td>
...
export default function App(){
  ...
  const startGame=useCallback(()=>{
    clearTimeout(autoRef.current); resetWarSlots(); setConfirmRestart(false); setSurrenderModal(false); gWinRef.current=null; pendingWarWinRef.current=null; setLastRig(0); pWarWinsRef.current=0; cWarWinsRef.current=0; setLongestSiege(0); setMercyHits(0); setRoyalsCaptured(0); playerTapsRef.current=0; warCardsWonRef.current=0;
  ...
  return(
    <div style={{height:"100vh",maxWidth:480,margin:"0 auto",overflow:"hidden",background:"linear-gradient(180deg,#06030f,#150a03,#0a0614,#06030f)",color:"#e8d5a3",display:"flex",flexDirection:"column",position:"relative",fontFamily:"'Segoe UI',Verdana,Geneva,sans-serif"}}>
      ...
      {showStats&&<StatsPanel dur={dur} fmt={fmt} fights={fights} wars={wars} pWarWins={pWarWins} cWarWins={cWarWins} longestSiege={longestSiege} mercyHits={mercyHits} royalsCaptured={royalsCaptured} playerTaps={playerTapsRef.current} warCardsWon={warCardsWonRef.current} pTotal={pDeck.length+pCollect.length} cTotal={cDeck.length+cCollect.length} onClose={()=>setShowStats(false)}/>}
