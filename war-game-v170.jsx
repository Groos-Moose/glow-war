...
// ╔══════════════════════════════════════════════════════════════════════════╗
// ║  THEME — Edit this block to change the game's color scheme              ║
// ╚══════════════════════════════════════════════════════════════════════════╝
const THEME={
  playerAccent:"#DAA520",           // gold — HUD, borders, card pile accents
  playerAccentSoft:"rgba(218,165,32,.25)",
  bfBg:"linear-gradient(180deg,rgba(14,6,30,.97),rgba(30,14,6,.97),rgba(14,6,30,.97))",
  duelBorder:"rgba(218,165,32,.25)",
  duelGlow:"0 0 12px rgba(0,0,0,.6)",
  duelSymbolColor:"rgba(218,165,32,.6)",// glowing swords color
  warBorder:"#f33",
  warBg:"rgba(80,0,0,.35)",
  warGlow:"0 0 28px 8px rgba(255,50,50,.28)",
  warColor:"#ff5555",
  warTextGlow:"0 0 10px #ff3333",
  warPathBorder:"rgba(255,80,80,.2)",
  doubleWarBorder:"#ff9900",
  doubleWarBg:"rgba(80,30,0,.4)",
  doubleWarGlow:"0 0 28px 8px rgba(255,140,0,.3)",
  doubleWarColor:"#ffaa00",
  doubleWarTextGlow:"0 0 10px #ffaa00",
  doubleWarPathBorder:"rgba(255,140,0,.25)",
  cardFaceBg:"linear-gradient(145deg,#fefaf0,#f5e8cc)",
  glowWin:{shadow:"0 0 18px 7px #22c55e,0 0 36px 12px rgba(34,197,94,.4),0 4px 12px rgba(0,0,0,.6)",border:"#22c55e"},
  glowLose:{shadow:"0 0 18px 7px #e74c3c,0 0 36px 12px rgba(231,76,60,.4),0 4px 12px rgba(0,0,0,.6)",border:"#e74c3c"},
  glowDw2:{shadow:"0 0 18px 7px #cc44ff,0 0 36px 12px rgba(180,0,255,.4),0 4px 12px rgba(0,0,0,.6)",border:"#cc44ff"},
  glowTie:{shadow:"0 0 18px 7px #f97316,0 0 36px 12px rgba(249,115,22,.4),0 4px 12px rgba(0,0,0,.6)",border:"#f97316"},
};
const WAR_SYMBOL="🔱"; // War symbol per round — change this to customize all war displays
const DECK_BACKS={
  purple:{bg:"linear-gradient(135deg,#1a0030,#3a006a,#1a0030)",border:"#7c3fbf",pat:"repeating-linear-gradient(45deg,transparent,transparent 4px,rgba(255,255,255,.04) 4px,rgba(255,255,255,.04) 8px)",sym:""},
  navy:  {bg:"linear-gradient(135deg,#000820,#001a50,#000820)",border:"#2244cc",pat:"repeating-linear-gradient(45deg,transparent,transparent 4px,rgba(255,255,255,.04) 4px,rgba(255,255,255,.04) 8px)",sym:""},
  forest:{bg:"linear-gradient(135deg,#001a08,#003d14,#001a08)",border:"#1a7c3a",pat:"repeating-linear-gradient(45deg,transparent,transparent 4px,rgba(255,255,255,.04) 4px,rgba(255,255,255,.04) 8px)",sym:""},
  crimson:{bg:"linear-gradient(135deg,#200005,#480010,#200005)",border:"#cc2233",pat:"repeating-linear-gradient(45deg,transparent,transparent 4px,rgba(255,255,255,.04) 4px,rgba(255,255,255,.04) 8px)",sym:""},
  midnight:{bg:"linear-gradient(135deg,#050510,#0f1030,#050510)",border:"#334488",pat:"repeating-linear-gradient(45deg,transparent,transparent 4px,rgba(255,255,255,.04) 4px,rgba(255,255,255,.04) 8px)",sym:""},
};
let DECK_BACK=DECK_BACKS.purple; // mutated each render by App — safe because React renders synchronously
...
function Card({card,faceDown=false,glow=null,small=false,med=false,sx={}}){
  ...
  const shadow=glow==="win"?THEME.glowWin.shadow:glow==="lose"?THEME.glowLose.shadow:glow==="dw2"?THEME.glowDw2.shadow:glow==="tie"?THEME.glowTie.shadow:"0 4px 14px rgba(0,0,0,.65)";
  const bc=glow==="win"?THEME.glowWin.border:glow==="lose"?THEME.glowLose.border:glow==="dw2"?THEME.glowDw2.border:glow==="tie"?THEME.glowTie.border:"rgba(139,90,43,.35)";
  ...
  return(<div style={{...base,background:THEME.cardFaceBg}}>
...
// ── MERCY SLOT ───────────────────────────────────────────────────────────────
// EKG_WIDTH_PCT: percentage of the slot width used by the EKG animation
const EKG_WIDTH_PCT=50;
// EkgSlot: shown for the player who is SHORT on cards (near-death indicator)
function EkgSlot(){
  const W=60,H=87;
  const ekgW=Math.round(W*EKG_WIDTH_PCT/100);
  const sw=ekgW*2; const sh=18; const mid=sh/2;
  const q=ekgW*0.25,r=ekgW*0.38,s=ekgW*0.45,t=ekgW*0.55,u=ekgW*0.62;
  const pts=[
    `0,${mid}`,`${q},${mid}`,`${r},${mid-7}`,`${s},${mid+8}`,`${t},${mid-4}`,`${u},${mid}`,`${ekgW},${mid}`,
    `${ekgW+q},${mid}`,`${ekgW+r},${mid-7}`,`${ekgW+s},${mid+8}`,`${ekgW+t},${mid-4}`,`${ekgW+u},${mid}`,`${sw},${mid}`,
  ].join(" ");
  return(
    <div style={{width:W,height:H,borderRadius:7,flexShrink:0,
      border:"2px dashed rgba(255,80,80,.55)",background:"rgba(255,30,30,.07)",
      display:"flex",alignItems:"center",justifyContent:"center",
      boxShadow:"0 0 8px 2px rgba(255,60,60,.15) inset"}}>
      <div style={{width:ekgW,height:sh,overflow:"hidden",flexShrink:0}}>
        <svg width={sw} height={sh} viewBox={`0 0 ${sw} ${sh}`}
          style={{display:"block",animation:"ekgScroll 1.4s linear infinite"}}>
...
// AngelSlot: shown for the player who HAS enough cards (protected by Mercy Rule)
function AngelSlot(){
  return(
    <div style={{width:60,height:87,borderRadius:7,flexShrink:0,
      border:"2px dashed rgba(218,165,32,.35)",background:"rgba(218,165,32,.03)",
      display:"flex",alignItems:"center",justifyContent:"center",
      boxShadow:"0 0 10px 2px rgba(218,165,32,.07) inset"}}>
      <svg width={44} height={56} viewBox="0 0 44 56" fill="none">
        {/* Halo */}
        <ellipse cx="22" cy="8" rx="11" ry="3.5" stroke="#DAA520" strokeWidth="1.8" opacity=".85"/>
        <ellipse cx="22" cy="8" rx="11" ry="3.5" stroke="rgba(255,255,255,.3)" strokeWidth=".6"/>
        <ellipse cx="22" cy="8" rx="9" ry="2" fill="rgba(218,165,32,.07)"/>
        {/* Left wing — two arcs for depth */}
        <path d="M22,28 Q5,18 1,34 Q12,36 22,40" fill="rgba(218,165,32,.14)" stroke="#DAA520" strokeWidth="1.1" opacity=".75"/>
        <path d="M22,28 Q9,22 5,28 Q12,28 22,34" fill="rgba(218,165,32,.06)" stroke="rgba(218,165,32,.5)" strokeWidth=".6"/>
        {/* Right wing */}
        <path d="M22,28 Q39,18 43,34 Q32,36 22,40" fill="rgba(218,165,32,.14)" stroke="#DAA520" strokeWidth="1.1" opacity=".75"/>
        <path d="M22,28 Q35,22 39,28 Q32,28 22,34" fill="rgba(218,165,32,.06)" stroke="rgba(218,165,32,.5)" strokeWidth=".6"/>
        {/* Angel body */}
        <circle cx="22" cy="21" r="5" fill="rgba(218,165,32,.12)" stroke="#DAA520" strokeWidth="1.1" opacity=".8"/>
        {/* Halo glow */}
        <ellipse cx="22" cy="8" rx="14" ry="5" fill="rgba(218,165,32,.04)"/>
      </svg>
    </div>
  );
}
...
function WarPathRow({spoils,warCard,glow,isPlayer,fam,inWar,pathBg,pathBorder,revealed,warCollect,rw,pol,carpetDelta,warCardRef,onAnimDone,mercySpoils,mercySide,dw2Sp,dw2W,dw2Revealed,dw2Round,dimWar1Sp,dimWar1W,onWinCardClick}){
          ...
          return sl.card
            ?<div key={`wrap-${sl.k}`} ref={!sl.isSpoil?warCardRef:null} style={{
              ...
              onTransitionEnd={e=>{
                ...
                <div style={{filter:((sl.isSpoil?dimWar1Sp:dimWar1W)?"brightness(0.5)":undefined),
                  cursor:(!sl.isSpoil&&sl.g==="win"&&isPlayer&&onWinCardClick)?"pointer":undefined}}
                  onClick={(!sl.isSpoil&&sl.g==="win"&&isPlayer&&onWinCardClick)?onWinCardClick:undefined}>
                ...
                {inDw2&&(sl.isSpoil?(dw2Sp?.[sl.revealIdx]!=null):(dw2W!=null))&&(
                  <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    {sl.isSpoil
                      ...
                      :<Card card={dw2W} faceDown={false} glow={dw2W&&rw?(rw==="tie"?"tie":(isPlayer?(rw==="player"?"win":"lose"):(rw==="cpu"?"win":"lose"))):null} med/>
                    }
                  </div>
                )}
            ...
            :(inWar?(()=>{
              ...
              if(isMercy){
                const mySide=isPlayer?"player":"cpu";
                const isShort=mercySide?mercySide===mySide:true;
                return isShort?<EkgSlot key={sl.k}/>:<AngelSlot key={sl.k}/>;
              }
...
function Battlefield({bf,cpuFam,plFam,quip,pol,pCollectRef:pCRef,cCollectRef:cCRef,dealIntro,introDelt,introTotal,onDealClick,onAnimDone,animStepRef,CW=60,CH=87,csSmall=false,csMed=false,csLg=true,pTotal=0,cTotal=0,potCards=0,onTapPile,onWinCardClick}){
  ...
  const {phase,cpuF,plF,cpuSp,plSp,cpuW,plW,rw,revealed,pot,warCollect,warInfo,fightSlide,mercySpoils,mercySide,dw2PlSp,dw2CpuSp,dw2PlW,dw2CpuW,dw2Round}=bf;
  ...
  const tieJustDeclared=phase==="war"&&plF&&cpuF&&plSp.length===0;
  const fpg=tieJustDeclared?"tie":(phase==="resolve"&&plF&&cpuF?(RV[plF.rank]>RV[cpuF.rank]?"win":null):null);
  const fcg=tieJustDeclared?"tie":(phase==="resolve"&&plF&&cpuF?(RV[cpuF.rank]>RV[plF.rank]?"win":null):null);
  ...
  // dimDuel: 50% brightness on plF/cpuF — only AFTER first war spoil placed (not on tie declaration)
  const dimDuel=((phase==="war"||phase==="war_card")&&plSp.length>0)||["war_reveal","double_war","double_war_card","double_reveal"].includes(phase);
  // dimWar1Sp: 50% on war-1 spoils from double_war start
  const dimWar1Sp=["double_war","double_war_card","double_reveal"].includes(phase);
  // dimWar1W: 50% on war-1 WAR CARD — only after first dw2 spoil placed
  const hasDw2Spoils=(dw2PlSp?.length||0)>0;
  const dimWar1W=dimWar1Sp&&(hasDw2Spoils||phase==="double_war_card"||phase==="double_reveal");
  const borderCol=inDouble?THEME.doubleWarBorder:inWar?THEME.warBorder:THEME.duelBorder;
  const glowShadow=inDouble?THEME.doubleWarGlow:inWar?THEME.warGlow:THEME.duelGlow;
  const warAccent=inDouble?THEME.doubleWarColor:THEME.warColor;
  const warGlow=inDouble?THEME.doubleWarTextGlow:THEME.warTextGlow;
  const pathBg=inWar?(inDouble?THEME.doubleWarBg:THEME.warBg):"rgba(255,255,255,.015)";
  const pathBorder=inWar?(inDouble?THEME.doubleWarPathBorder:THEME.warPathBorder):"rgba(255,255,255,.05)";


  return(
    <div style={{
      background:THEME.bfBg,
        ...
        <div style={{width:"22%",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2}}>
          ...
          <div ref={cpuFightRef} style={{position:"relative",width:CW,height:CH}}>
            ...
            {cpuF&&<div style={{position:"absolute",inset:0,
              ...
              filter:dimDuel?"brightness(0.5)":undefined}}
        ...
        <div style={{width:"50%",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3}}>
          <div style={{height:csSmall?44:csMed?56:68,width:"100%",display:dealIntro?"none":"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
            ...
            {(()=>{
              ...
              const numSymbols=warRound+1;
              const symFz=Math.max(9,Math.round(iSz*1.5/(numSymbols+0.5)));
              const symLine=Array.from({length:numSymbols},()=>WAR_SYMBOL).join(" ");
              if(inWar) return(
                <div key={`wa-${dw2Round}`} style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontWeight:"bold",letterSpacing:lSp,color:warAccent,textShadow:warGlow,animation:showStats?"none":"warAlert 1.8s ease-out forwards",textAlign:"center",lineHeight:1.4,opacity:showStats?.25:1,transition:"opacity .4s"}}>
                  <div style={{fontSize:symFz,lineHeight:1,marginBottom:1,whiteSpace:"nowrap"}}>{symLine}</div>
              ...
              return(
                <div style={{fontWeight:"bold",letterSpacing:lSp,color:"rgba(218,165,32,.35)",textAlign:"center",lineHeight:1.3}}>
                  <div style={{lineHeight:1,marginBottom:1,display:"flex",justifyContent:"center"}}>
                    <svg width={iSz} height={iSz} viewBox="0 0 36 36" style={{filter:`drop-shadow(0 0 4px ${THEME.duelSymbolColor}) drop-shadow(0 0 10px ${THEME.duelSymbolColor})`,overflow:"visible"}}>
                      <line x1="4" y1="4" x2="32" y2="32" stroke="#fffbe0" strokeWidth="2.8" strokeLinecap="round"/>
                      <line x1="4" y1="10" x2="10" y2="4" stroke="#DAA520" strokeWidth="2" strokeLinecap="round"/>
                      <line x1="32" y1="4" x2="4" y2="32" stroke="#fffbe0" strokeWidth="2.8" strokeLinecap="round"/>
                      <line x1="26" y1="4" x2="32" y2="10" stroke="#DAA520" strokeWidth="2" strokeLinecap="round"/>
                      <circle cx="18" cy="18" r="2.2" fill="#ffffff" opacity=".95"/>
                    </svg>
                  </div>
        ...
        <div style={{width:"22%",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2}}>
          ...
          <div ref={plFightRef} style={{position:"relative",width:CW,height:CH}}>
            ...
            {plF&&<div style={{position:"absolute",inset:0,
              ...
              cursor:(fpg==="win"&&onWinCardClick)?"pointer":undefined,
              filter:dimDuel?"brightness(0.5)":undefined}}
              ...
              onClick={(fpg==="win"&&onWinCardClick)?onWinCardClick:undefined}>
...
function GameOverOverlay({gWin,gWinReason,gameMode,warTarget,cpuFam,playerName,dur,fmt,fights,wars,pWarWins,cWarWins,onRestart,sessionStats,onShowStats}){
  ...
  return(
    <div style={{position:"absolute",inset:0,zIndex:100,background:isWin?"radial-gradient(ellipse at center,rgba(0,60,0,.97),rgba(0,0,0,.97))":"radial-gradient(ellipse at center,rgba(60,0,0,.97),rgba(0,0,0,.97))",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,overflowY:"auto",padding:"16px 0"}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        <button onClick={onShowStats} style={{background:"rgba(138,92,246,.15)",border:"1.5px solid rgba(138,92,246,.5)",color:"#c084fc",padding:"10px 16px",borderRadius:10,cursor:"pointer",fontSize:13,fontWeight:"bold",letterSpacing:1,boxShadow:"0 0 12px rgba(138,92,246,.2)"}}>📊 Stats</button>
  ...
  return(
    <div style={{position:"absolute",inset:0,zIndex:200,background:"rgba(0,0,0,.92)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"linear-gradient(160deg,#1a0f05,#0f0a1a)",border:"2px solid rgba(218,165,32,.5)",borderRadius:18,padding:"22px 20px",width:"88%",maxWidth:340,maxHeight:"90vh",overflowY:"auto",position:"relative"}}>
        ...
        <div style={{fontSize:9,color:"rgba(218,165,32,.65)",textAlign:"center",letterSpacing:1.5,marginBottom:14}}>v170</div>
        ...
        <CRow label="Card Back">
          <div style={{display:"flex",gap:5,alignItems:"center"}}>
            {Object.entries(DECK_BACKS).map(([key,db])=>(
              <button key={key} onClick={()=>upd({cardBack:key})} title={key.charAt(0).toUpperCase()+key.slice(1)}
                style={{width:24,height:34,borderRadius:4,padding:0,cursor:"pointer",
                  background:db.bg,
                  border:`2px solid ${config.cardBack===key?db.border:"rgba(255,255,255,.12)"}`,
                  boxShadow:config.cardBack===key?`0 0 8px 2px ${db.border}66`:"none",
                  transition:"all .2s"}}/>
            ))}
          </div>
        </CRow>
...
function StatsPanel({dur,fmt,fights,wars,pWarWins,cWarWins,longestSiege,mercyHits,royalsCaptured,playerTaps,warCardsWon,pTotal,cTotal,ptsCaptured,ptsLost,maxPtsCap,maxPtsLost,evtsCap,evtsLost,sessionStats,onClose}){
  const avgPtsCap=evtsCap>0?Math.round(ptsCaptured/evtsCap):0;
  const avgPtsLost=evtsLost>0?Math.round(ptsLost/evtsLost):0;
  const classicGames=sessionStats.classic.totalGames;
  const raceGames=sessionStats.race.totalGames;
  const rows=[
    {cat:"VITALITY",label:"Remaining Lifespan",val:pTotal<=0?"--":`${pTotal} cards (${Math.round(pTotal/52*100)}%)`,tip:"How many cards you have left out of 52. When this hits 0 you lose."},
    {cat:"VITALITY",label:"Losing Risk",val:pTotal<=5?"CRITICAL":pTotal<=13?"HIGH":pTotal<=26?"MODERATE":"LOW",tip:"Estimated danger based on your remaining card count vs 52 total."},
    {cat:"VITALITY",label:"Heartbeat Spikes",val:`${mercyHits} mercy event${mercyHits!==1?"s":""}`,tip:"How many times the Mercy Rule saved you from running out of cards during a war."},
    {cat:"COMBAT",label:"Duels Fought",val:`${fights}`,tip:"Total head-to-head card comparisons played this game."},
    {cat:"COMBAT",label:"Wars Won",val:`${pWarWins} of ${wars} wars`,tip:"Wars you won vs total wars declared. A war starts when both players tie on rank."},
    {cat:"COMBAT",label:"War Win Rate",val:wars>0?`${Math.round(pWarWins/wars*100)}%`:"--",tip:"Percentage of wars you've won: Wars Won ÷ Total Wars × 100."},
    {cat:"COMBAT",label:"Longest Siege",val:longestSiege>0?`${longestSiege} cards in pot`:"-",tip:"Largest card pot in a single war — total cards on the table including spoils and war cards."},
    {cat:"COMBAT",label:"Royals Captured",val:`${royalsCaptured}`,tip:"J, Q, and K cards you've taken from the CPU. Face cards are worth more points."},
    {cat:"POINTS",label:"Max Points Captured",val:maxPtsCap>0?`${maxPtsCap}pts`:"--",tip:"Highest point value won in a single duel or war. Card point values: 2=2 … 10=10, J=11, Q=12, K=13, A=14."},
    {cat:"POINTS",label:"Max Points Lost",val:maxPtsLost>0?`${maxPtsLost}pts`:"--",tip:"Highest point value lost in a single duel or war."},
    {cat:"POINTS",label:"Avg Points Captured",val:avgPtsCap>0?`${avgPtsCap}pts/event`:"--",tip:"Average point value won per winning event (duel or war): Total Points Captured ÷ Win Count."},
    {cat:"POINTS",label:"Avg Points Lost",val:avgPtsLost>0?`${avgPtsLost}pts/event`:"--",tip:"Average point value lost per losing event (duel or war): Total Points Lost ÷ Loss Count."},
    {cat:"LUCK",label:"Luck Quotient",val:(()=>{const base=fights>0?Math.round((pWarWins/Math.max(wars,1))*50+(royalsCaptured/Math.max(fights,1))*100):0;return fights>0?`${Math.min(base,99)}/99`:"--";})(),tip:"Composite luck score 0–99 based on your war win rate and how many royals you've captured. Higher = luckier."},
    {cat:"LUCK",label:"Suit Dominance",val:(()=>{return pWarWins>cWarWins?"Offensive":"Defensive";})(),tip:"Are you winning more wars than the CPU (Offensive) or fewer (Defensive)?"},
    {cat:"PERFORMANCE",label:"CPM (Clicks/Min)",val:dur>0?`${Math.round(playerTaps/(dur/60))}`:"-",tip:"Your tap rate: total taps ÷ elapsed minutes. Higher means you play faster."},
    {cat:"PERFORMANCE",label:"Card Point Value",val:`${warCardsWon} cards won in war`,tip:"Total number of cards you've captured through wars (not duels)."},
    {cat:"PERFORMANCE",label:"War Efficiency",val:wars>0&&warCardsWon>0?`${Math.round(warCardsWon/wars)} cards/war avg`:"--",tip:"Average cards captured per war: War Cards Won ÷ Total Wars."},
    {cat:"SESSION",label:"Winning Streak",val:`${sessionStats.winStreak} (best: ${sessionStats.bestStreak})`,tip:"How many games in a row you've won this session. Best is your longest streak so far."},
    {cat:"SESSION",label:"Classic Avg Time",val:classicGames>0?fmt(Math.round(sessionStats.classic.totalTime/classicGames)):"--",tip:"Average duration of Classic mode games you've played this session."},
    {cat:"SESSION",label:"War Race Avg Time",val:raceGames>0?fmt(Math.round(sessionStats.race.totalTime/raceGames)):"--",tip:"Average duration of War Race games you've played this session."},
  ];
  let lastCat="";
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.72)"}} onClick={onClose}>
      <div style={{background:"linear-gradient(160deg,#0f0a1a,#1a0f2e)",border:"1.5px solid rgba(138,92,246,.4)",borderRadius:14,padding:"18px 20px",minWidth:320,maxWidth:420,width:"90%",boxShadow:"0 0 40px rgba(138,92,246,.3)",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{fontSize:13,fontWeight:"bold",color:"#c084fc",letterSpacing:1.5,textTransform:"uppercase"}}>Game Stats</div>
          <div style={{fontSize:9,color:"#a0a0c0"}}>{fmt(dur)}</div>
          <button onClick={onClose} style={{background:"none",border:"none",color:"rgba(200,180,255,.6)",cursor:"pointer",fontSize:16,lineHeight:1,padding:2}}>✕</button>
        </div>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
          <tbody>
            {rows.map((r,i)=>{
              const isNew=r.cat!==lastCat;
              lastCat=r.cat;
              return(
                <React.Fragment key={i}>
                  {isNew&&<tr><td colSpan={2} style={{paddingTop:i>0?10:0,paddingBottom:4,fontSize:9,fontWeight:"bold",color:"rgba(192,132,252,.7)",letterSpacing:2,textTransform:"uppercase",borderBottom:"1px solid rgba(138,92,246,.15)"}}>{r.cat}</td></tr>}
                  <tr style={{background:i%2===0?"rgba(255,255,255,.02)":"transparent"}} title={r.tip}>
                    <td style={{padding:"4px 6px",color:"rgba(200,190,220,.7)",fontSize:10,cursor:"help"}} title={r.tip}>{r.label}</td>
                    <td style={{padding:"4px 6px",color:"#e2d9f3",fontWeight:"bold",textAlign:"right",fontSize:11}}>{r.val}</td>
                  </tr>
                </React.Fragment>
              );
            })}
        ...
        <div style={{marginTop:10,fontSize:8,color:"rgba(255,255,255,.2)",textAlign:"center",fontStyle:"italic"}}>Hover a stat name for explanation</div>
      </div>
    </div>
  );
}

const INIT_BF={phase:"idle",cpuF:null,plF:null,cpuSp:[],plSp:[],cpuW:null,plW:null,rw:null,revealed:false,tapCt:0,pot:0,maxSp:0,warCollect:null,warInfo:null,fightSlide:null,mercySpoils:0,mercySide:null,dw2PlSp:[],dw2CpuSp:[],dw2PlW:null,dw2CpuW:null,dw2Round:0};
...
export default function App(){
  ...
  const [config,setConfig]=useState(()=>{
    ...
    const defaults={muted:true,volume:.7,pileSide:"right",showMsg:true,autoSpeed:0,gameMode:"cards",warTarget:3,warAutoFill:false,logEvents:true,cardSize,showDevBar:true,cardBack:"purple"};
    try{const s=localStorage.getItem("glowWar_settings");if(s){return {...defaults,...JSON.parse(s)};}}catch(e){}
    return defaults;
  });
  useEffect(()=>{try{localStorage.setItem("glowWar_settings",JSON.stringify(config));}catch(e){}},[config]);
  ...
  const [longestSiege,setLongestSiege]=useState(0);
  const [mercyHits,setMercyHits]=useState(0);
  const [royalsCaptured,setRoyalsCaptured]=useState(0);
  const [ptsCaptured,setPtsCaptured]=useState(0);
  const [ptsLost,setPtsLost]=useState(0);
  const [maxPtsCap,setMaxPtsCap]=useState(0);
  const [maxPtsLost,setMaxPtsLost]=useState(0);
  const evtsCapRef=useRef(0);
  const evtsLostRef=useRef(0);
  const playerTapsRef=useRef(0);
  const warCardsWonRef=useRef(0);
  const skipDealAfterRef=useRef(false);
  const pendingRigRef=useRef(0); // depth of next-deal rig (0=none); consumed inline by dealFight
  ...
  const [sessionStats,setSessionStats]=useState({
    classic:{wins:0,losses:0,duels:0,wars:0,bestTime:null,totalTime:0,totalGames:0},
    race:{wins:0,losses:0,duels:0,wars:0,bestTime:null,totalTime:0,totalGames:0},
  ...
  // handleRigBtn: arms WARx buttons.
  // For idle/resolve/collecting/war_reveal/double_reveal: sets pendingRigRef so dealFight
  //   applies the rig inline (avoids autoplay race condition where deck state update lags).
  // For mid-war phases: immediate deck rig for the remaining war card slot(s).
  ...
  const handleRigBtn=useCallback((depth)=>{
    ...
    if(ph==='idle'||ph==='resolve'||ph==='collecting'||ph==='war_reveal'||ph==='double_reveal'){
      // Arm pending rig — dealFight will apply it inline on the next fight deal
      pendingRigRef.current=depth;
      if(addLogRef.current) addLogRef.current("dev",`WARx${depth} armed — pending for next deal (phase:${ph})`);
    } else {
      // Mid-war: immediate rig for remaining war card position(s)
      const n=warSlotRef.current;
      const m=dw2SlotRef.current;
      let slots;
      if(ph==='war'){
        const wc=3-n;
      ...
      if(slots.length===0){
        if(addLogRef.current) addLogRef.current("dev",`WARx${depth} armed — no mid-war slots in phase:${ph}`);
        return;
      }
      const needed=Math.max(...slots)+1;
      const doRig=()=>{
        const s=rigDecks(slots);
        if(addLogRef.current) addLogRef.current("dev",`WARx${depth} mid-war rig (${s} slot${s!==1?"s":""})`);
      };
      let needsWait=false;
  ...
  useEffect(()=>{
    ...
    if(started&&!closed&&!showConfig&&!showStats){
      ctRef.current=setInterval(()=>{
        if(gWinRef.current){clearInterval(ctRef.current);return;}
        setDur(d=>d+1);
      },1000);
    }
    return()=>clearInterval(ctRef.current);
  },[started,closed,showConfig,showStats]);

  const fmt=s=>{const h=Math.floor(s/3600);const m=Math.floor((s%3600)/60);const ss=s%60;return h>0?`${h}:${String(m).padStart(2,"0")}:${String(ss).padStart(2,"0")}`:`${String(m).padStart(2,"0")}:${String(ss).padStart(2,"0")}`;}
  ...
  const dealFight=useCallback((pd,cd,pot,potCards)=>{
    ...
    // Apply pending WARx rig inline — fires even if autoplay called dealFight before state settled
    const rigDepth=pendingRigRef.current;
    if(rigDepth>0){
      pendingRigRef.current=0;
      pd=[...pd]; // copy before swapping
      const rigSlots=[0,rigDepth>=2?4:-1,rigDepth>=3?8:-1].filter(i=>i>=0&&i<pd.length&&i<cd.length);
      const usedIdxs=new Set();
      for(const i of rigSlots){
        const tr=cd[i].rank;
        if(pd[i].rank===tr){usedIdxs.add(i);continue;}
        const si=pd.findIndex((c,j)=>!usedIdxs.has(j)&&j!==i&&c.rank===tr);
        if(si>=0){[pd[i],pd[si]]=[pd[si],pd[i]];usedIdxs.add(i);}
      }
      if(addLogRef.current) addLogRef.current("dev",`WARx${rigDepth} inline rig applied in dealFight`);
    }
    ...
    if(pv===cv){
      ...
      if(pTotalAfter===0||cTotalAfter===0){
        ...
        const merSide=warWinner==="player"?"cpu":"player"; // short side loses
        if(warWinner==="player") setMercyHits(n=>n+1);
        wrapBfState({phase:"war_reveal",cpuF:cc,plF:pc,cpuSp:[],plSp:[],cpuW:cc,plW:pc,rw:warWinner,revealed:true,tapCt:0,pot:pot+2,maxSp:0,warInfo:wi,warCollect:null,fightSlide:null,mercySpoils:-1,mercySide:merSide});
        setWCards(allCards);
        setMsg(warWinner==="player"?`Mercy rule — ${cfRef.current.name} ran out of cards!`:"Mercy rule — you ran out of cards!");
        snd("war");
      ...
      wrapBfState({phase:"war",cpuF:cc,plF:pc,cpuSp:[],plSp:[],cpuW:null,plW:null,rw:null,revealed:false,tapCt:0,pot:pot+2,maxSp:3,warCollect:null,fightSlide:null,warInfo:null,mercySpoils:0,dw2PlSp:[],dw2CpuSp:[],dw2PlW:null,dw2CpuW:null,dw2Round:0});
        setMsg("WAR! Place 3 spoils then your war card.");
        snd("war");
      }
    } else {
      ...
      const duelPts=pv+cv+(potCards?cardValue(potCards):0);
      if(w==="player"){
        setMsg(`You win! ${pc.rank}(${pv}) vs ${cc.rank}(${cv}).`);
        if(["J","Q","K"].includes(cc.rank)) setRoyalsCaptured(r=>r+1);
        setPtsCaptured(x=>x+duelPts);setMaxPtsCap(x=>Math.max(x,duelPts));evtsCapRef.current++;
      }else{
        setMsg(`${cfRef.current.name} wins! ${cc.rank}(${cv}) vs ${pc.rank}(${pv}).`);
        setPtsLost(x=>x+duelPts);setMaxPtsLost(x=>Math.max(x,duelPts));evtsLostRef.current++;
      }
      snd("clash");
  ...
  const warStep=useCallback(()=>{
    ...
    if(pd.length===0||cd.length===0){
      ...
      const shortName=pd.length===0?"You":cfRef.current.name;
      const winnerName=ew==="player"?"You":cfRef.current.name;
      setMsg(`Tie! ${shortName} has no cards for war — ${winnerName} takes the pot.`);
    ...
    if(autoFill){
      ...
      if(warSlotRef.current<3){
        ...
        const autoMercySide=pd.length<=1?"player":"cpu";
        if(autoMercySide==="cpu") setMercyHits(n=>n+1); // player survived
        wrapBfState(s=>({...s, mercySpoils:warSlotRef.current, mercySide:autoMercySide, plSp:[...ps], cpuSp:[...cs]}));
      }
    ...
    // Capture mercy info BEFORE resetWarSlots (manual mode: mercySpoils never set earlier)
    const spoilsPlaced=warSlotRef.current;
    const needsMercy=spoilsPlaced<3;
    const mercySideVal=needsMercy?(pd.length<=1?"player":"cpu"):null;
    ...
    setWars(x=>x+1);
    setLongestSiege(s=>Math.max(s,wc.length));
    if(playerWon){
      warCardsWonRef.current+=wc.length;
      // Count CPU's face cards (J/Q/K) captured
      const cpuRoyals=[cur.cpuF,...(cs||[]),b].filter(c=>c&&["J","Q","K"].includes(c.rank)).length;
      if(cpuRoyals>0) setRoyalsCaptured(r=>r+cpuRoyals);
      const warPts=cardValue(wc);setPtsCaptured(x=>x+warPts);setMaxPtsCap(x=>Math.max(x,warPts));evtsCapRef.current++;
      ...
      const warPts2=cardValue(wc);setPtsLost(x=>x+warPts2);setMaxPtsLost(x=>Math.max(x,warPts2));evtsLostRef.current++;
    ...
    if(!isDouble){
      ...
      wrapBfState(s=>({...s,
        ...
        mercySpoils:needsMercy?spoilsPlaced:0,mercySide:needsMercy?mercySideVal:null,
        dw2PlSp:[],dw2CpuSp:[],dw2PlW:null,dw2CpuW:null,dw2Round:0,
      }));
      setMsg(playerWon
        ?`You captured ${wi.count} cards · ${wi.totalVal}pts`
        :`${cfRef.current.name} captured ${wi.count} cards · ${wi.totalVal}pts`);
  ...
  const collectAndDeal=useCallback((pd,cd,winner,cards,pc,cc,topCard)=>{
    ...
    setWCards([]);
    setTick(t=>t+1);
    // Check if a win-card-click requested collect-only (no auto-deal)
    const skipDeal=skipDealAfterRef.current;
    skipDealAfterRef.current=false; // consume the flag
    if(skipDeal){
      // Flip if needed, then go to idle — player taps draw pile to deal
      let idlePd=finalPd, idleCd=finalCd;
      if(idlePd.length===0&&newPc.length>0){idlePd=shuf([...newPc]);setPDeck(idlePd);setPCollect([]);}
      if(idleCd.length===0&&newCc.length>0){idleCd=shuf([...newCc]);setCDeck(idleCd);setCCollect([]);}
      wrapBfState({...INIT_BF,phase:"idle"});
      return;
    }
  ...
  // tapWinCard: clicking the winning card collects but does NOT auto-deal the next fight.
  // skipDealAfterRef is set before calling tapPile — collectAndDeal reads and consumes it.
  const tapWinCard=useCallback(()=>{
    skipDealAfterRef.current=true;
    tapPileRef.current();
  },[]);
  const tapWinCardRef=useRef(tapWinCard);
  tapWinCardRef.current=tapWinCard;
  ...
  const recordResult=useCallback((winner)=>{
    ...
    setSessionStats(prev=>{
      ...
      const newBest=winner==="player"
        ...
        :prev_mode.bestTime;
      const newStreak=winner==="player"?prev.winStreak+1:0;
      return {...prev,[mode]:{
        ...
        bestTime:newBest,
        totalTime:prev_mode.totalTime+durRef.current,
        totalGames:prev_mode.totalGames+1,
      },winStreak:newStreak,bestStreak:Math.max(prev.bestStreak,newStreak)};
    });
  },[]);

  const startGame=useCallback(()=>{
    clearTimeout(autoRef.current); resetWarSlots(); setConfirmRestart(false); setSurrenderModal(false); gWinRef.current=null; pendingWarWinRef.current=null; pendingRigRef.current=0; setLastRig(0); pWarWinsRef.current=0; cWarWinsRef.current=0; setLongestSiege(0); setMercyHits(0); setRoyalsCaptured(0); playerTapsRef.current=0; warCardsWonRef.current=0; setPtsCaptured(0); setPtsLost(0); setMaxPtsCap(0); setMaxPtsLost(0); evtsCapRef.current=0; evtsLostRef.current=0;
  ...
  useEffect(()=>{
    ...
    if(!active || !started || phase==="gameover" || showConfig || showStats){
  ...
  },[phase, tick, dev, autoSpeed, started, config.warAutoFill, showConfig, showStats]);

  // ── HUD display values ────────────────────────────────────────────────────
  const pol=config.pileSide==="right";
  // Card size props derived from config
  // Update active deck back from config (safe: React renders synchronously)
  DECK_BACK = DECK_BACKS[config.cardBack||"purple"] || DECK_BACKS.purple;
  ...
  const pileOk=phase!=="gameover"&&(!gWinRef.current||!!pendingWarWinRef.current); // pendingWarWin: allow tap so race-win tap reaches tapPile→gameover
  ...
  if(closed) return(
    <div style={{maxWidth:480,margin:"0 auto",fontFamily:"'Segoe UI',Verdana,Geneva,sans-serif"}}>
      <div style={{background:"rgba(0,0,0,.9)",border:"1px solid rgba(218,165,32,.3)",borderRadius:10,padding:"10px 16px",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1,fontSize:11,color:"#DAA520",letterSpacing:1}}>GloWar</div>
  ...
  return(
    <div style={{height:"100vh",maxWidth:480,margin:"0 auto",overflow:"hidden",background:"linear-gradient(180deg,#06030f,#150a03,#0a0614,#06030f)",color:"#e8d5a3",display:"flex",flexDirection:"column",position:"relative",fontFamily:"'Segoe UI',Verdana,Geneva,sans-serif"}}>
      <style>{`
        ...
        @keyframes ekgScroll{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
      `}</style>
      ...
      <div style={{flexShrink:0,background:"rgba(0,0,0,.75)",borderBottom:"1px solid rgba(218,165,32,.2)"}}>
        <div style={{display:"flex",alignItems:"center",padding:"5px 8px",gap:4}}>
          ...
          <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
            <div style={{fontSize:11,color:"#DAA520",letterSpacing:2,fontWeight:"bold",lineHeight:1.2}}>GloWar</div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
            ...
            <button onClick={()=>setShowStats(s=>!s)}
              title="Game Stats"
              style={{width:26,height:26,flexShrink:0,background:showStats?"rgba(138,92,246,.2)":"rgba(0,0,0,.4)",border:showStats?"1.5px solid rgba(138,92,246,.7)":"1px solid rgba(255,255,255,.12)",borderRadius:6,cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",
              boxShadow:showStats?"0 0 6px 1px rgba(138,92,246,.4)":"none",
      ...
      {/* ══ DEV BAR — Easter egg: only visible when playerName is "Bruce_Groose" ══ */}
      {playerName==="Bruce_Groose"&&phase!=="gameover"&&(
      ...
      {phase==="gameover"&&<GameOverOverlay gWin={gWin} gWinReason={gWinReason} gameMode={config.gameMode} warTarget={config.warTarget} cpuFam={cpuFam} playerName={playerName} dur={dur} fmt={fmt} fights={fights} wars={wars} pWarWins={pWarWins} cWarWins={cWarWins} onRestart={()=>{if(pendingActionRef.current){const fn=pendingActionRef.current;pendingActionRef.current=null;fn();}else{startGame();}}} sessionStats={sessionStats} onShowStats={()=>setShowStats(true)}/>}
      ...
      {showStats&&<StatsPanel dur={dur} fmt={fmt} fights={fights} wars={wars} pWarWins={pWarWins} cWarWins={cWarWins} longestSiege={longestSiege} mercyHits={mercyHits} royalsCaptured={royalsCaptured} playerTaps={playerTapsRef.current} warCardsWon={warCardsWonRef.current} pTotal={pDeck.length+pCollect.length} cTotal={cDeck.length+cCollect.length} ptsCaptured={ptsCaptured} ptsLost={ptsLost} maxPtsCap={maxPtsCap} maxPtsLost={maxPtsLost} evtsCap={evtsCapRef.current} evtsLost={evtsLostRef.current} sessionStats={sessionStats} onClose={()=>setShowStats(false)}/>}
