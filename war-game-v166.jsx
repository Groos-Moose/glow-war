...
function EkgSlot({empty=false}){
  const W=60,H=87;
  // Visible EKG area width; SVG is 2× wide so the repeating pattern scrolls seamlessly
  const ekgW=Math.round(W*EKG_WIDTH_PCT/100); // 30px visible
  const sw=ekgW*2;   // SVG total width (2 repeats)
  const sh=18;       // SVG height
  const mid=sh/2;    // vertical center
  // One EKG cycle = ekgW px wide: flat → narrow spike → flat
  const q=ekgW*0.25, r=ekgW*0.38, s=ekgW*0.45, t=ekgW*0.55, u=ekgW*0.62;
  // Build two repeating cycles for seamless loop
  const pts=[
    `0,${mid}`,`${q},${mid}`,`${r},${mid-7}`,`${s},${mid+8}`,`${t},${mid-4}`,`${u},${mid}`,`${ekgW},${mid}`,
    `${ekgW+q},${mid}`,`${ekgW+r},${mid-7}`,`${ekgW+s},${mid+8}`,`${ekgW+t},${mid-4}`,`${ekgW+u},${mid}`,`${sw},${mid}`,
  ].join(" ");
  const borderCol=empty?"rgba(160,100,255,.5)":"rgba(255,80,80,.55)";
  const bgCol=empty?"rgba(80,0,160,.07)":"rgba(255,30,30,.07)";
  const glowCol=empty?"rgba(120,0,255,.12)":"rgba(255,60,60,.15)";
  const strokeCol=empty?"#cc44ff":"#ef4444";
  return(
    <div style={{width:W,height:H,borderRadius:7,flexShrink:0,
      border:`2px dashed ${borderCol}`,background:bgCol,
      display:"flex",alignItems:"center",justifyContent:"center",
      boxShadow:`0 0 8px 2px ${glowCol} inset`}}>
      <div style={{width:ekgW,height:sh,overflow:"hidden",position:"relative",flexShrink:0}}>
        <svg width={sw} height={sh} viewBox={`0 0 ${sw} ${sh}`}
          style={{display:"block",animation:"ekgScroll 1.4s linear infinite"}}>
          <polyline points={pts} fill="none" stroke={strokeCol} strokeWidth="1.5"
        ...
        {slots.map(sl=>{
          ...
          return sl.card
            ...
            :(inWar?(()=>{
              ...
              if(isMercy) return <EkgSlot key={sl.k} empty={mercySpoils===-1}/>;
  ...
  return(
    <div style={{position:"absolute",inset:0,zIndex:200,background:"rgba(0,0,0,.92)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"linear-gradient(160deg,#1a0f05,#0f0a1a)",border:"2px solid rgba(218,165,32,.5)",borderRadius:18,padding:"22px 20px",width:"88%",maxWidth:340,maxHeight:"90vh",overflowY:"auto",position:"relative"}}>
        ...
        <div style={{fontSize:9,color:"rgba(218,165,32,.65)",textAlign:"center",letterSpacing:1.5,marginBottom:14}}>v166</div>
